...
builtin_cd :: proc(args: []string, state: ^types.Session_State) {
	if len(args) == 1 {
		...
		if err == nil {
			...
			if cwd_err == nil {
				state.cur_dir = strings.clone(cwd, state.persistent_allocator)
			}
		}
		return
	}
	...
	case "..":
		...
		if len(parts) > 1 {
			...
			if os.chdir(new_dir) == nil {
				state.cur_dir = strings.clone(new_dir, state.persistent_allocator)
			}
		}
	case "/":
		os.chdir(args[1])
...
builtin_alias :: proc(args: []string, state: ^types.Session_State) {
	...
	alias_args := make([dynamic]string, allocator = state.persistent_allocator)
	for a in args[2:] {
		append(&alias_args, strings.clone(a, state.persistent_allocator))
	}

	state.aliases[strings.clone(args[1], state.persistent_allocator)] = types.Command {
		command   = strings.clone(args[2], state.persistent_allocator),
